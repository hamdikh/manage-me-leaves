...
public interface LeaveRequestRepository extends JpaRepository<LeaveRequest, UUID>, QuerydslPredicateExecutor<LeaveRequest> {
    ...
    @Query(value = "SELECT l FROM LeaveRequest l WHERE l.isDeleted = false AND l.status <> 'DRAFT' AND l.collaborator.salesManager.id = :id")
    ...
    @Query(value = "SELECT l.* FROM public.leave_requests AS l INNER JOIN public.business_units AS bu ON bu.business_unit_leader_id = l.collaborator_id WHERE l.is_deleted = 'false' AND AND l.status = 'VALIDATED' bu.id = :id", nativeQuery = true)
    List<LeaveRequest> findLeaveRequestsByBusinessUnitIdForBUM(UUID id);

    @Query(value = "SELECT l.* FROM public.leave_requests AS l INNER JOIN public.collaborators AS c ON c.id = l.collaborator_id WHERE l.is_deleted = 'false' AND l.status = 'VALIDATED' AND c.team_id = :id", nativeQuery = true)
    List<LeaveRequest> findLeaveRequestsByTeamId(UUID id);

    @Query(value = "SELECT l.* FROM public.leave_requests AS l INNER JOIN public.teams AS t ON t.team_leader_id = l.collaborator_id WHERE l.is_deleted = 'false' AND l.status = 'VALIDATED' AND t.id = :id", nativeQuery = true)
